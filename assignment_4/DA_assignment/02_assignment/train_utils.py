...
def train_adabn(model, source_loader, target_loader, args, device):
    ...
    for module in model.modules():
        if isinstance(module, nn.BatchNorm1d):
            ...
            module.momentum = None  # PyTorch default is 0.1
    ...
    # After adaptation, evaluate the model
    model.eval()  # Set model to evaluation mode
    with torch.no_grad():
        source_loss, source_accuracy = evaluate(model, source_loader, device)
        target_loss, target_accuracy = evaluate(model, target_loader, device)
    ...
    # Print loss and accuracy for source and target 
    print(f"After adaptation: Source Loss {source_loss:.4f}, Accuracy: {100 * source_accuracy:.2f}%")
    print(f"After adaptation: Target Loss {target_loss:.4f}, Accuracy: {100 * target_accuracy:.2f}%")
